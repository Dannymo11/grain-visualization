//-----------------------------------------------------------------------------
// Granular MIDI Synthesizer with Visual Grain Representation
// Combined audio+visual instrument - based on granular_midi.ck and visuals.ck
//-----------------------------------------------------------------------------
...
Event updateEvent;                    // Event to signal when parameters change
Event grainFired;                     // Event to trigger grain visualization
...
// Play a MIDI note
fun void playNote(int note, float velocity) {
    ...
    while (true) {
        ...
        while (min.recv(msg)) {
            ...
            if ((msg.data1 & 0xF0) == 0x90 && msg.data3 > 0) {
                ...
                // Debug
                <<< "Visualizing Note On:", note, "Velocity:", velocity >>>;
                
                // Create visualization for note start
                spork ~ playNote(note, velocity);
            }
